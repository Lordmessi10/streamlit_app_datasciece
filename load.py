...
st.header('Data obtained in year 2023')
...
try:
    ...
    select_job_titles = data[data['job_title'].isin(jobs_selection)]
    
    # table
    """### Top 5 rows"""
    st.dataframe(select_job_titles.head())
    ...
    st.line_chart(df)
    # bar chart is categorical while line is serias
    df= data.groupby('work_year')['salary_in_usd'].mean().sort_values(ascending=True)
    st.bar_chart(df)
